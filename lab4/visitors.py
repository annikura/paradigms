...
class PrettyPrinter(OperatorVisitor):
    ...
    def decrease_style(self):
        if self.style is SentenceEnum:
            self.style = PartialEnum
        elif self.style is PartialEnum:
            self.style = ArgsEnum

    def output(self, *args):
        print(self.style.beg * self.level, end='', sep='')
        for i, arg in enumerate(args):
            if arg is None:
                continue
            try:
                if i > 0:
                    print(self.style.sep, end='', sep='')
                arg.access(self)
            except AttributeError:
                if isinstance(arg, list):
                    ...
                    print(self.style.sep if i > 0 else '', arg, end='', sep='')
        print(end=self.style.end)

    def visitNumber(self, num):
        self.output(num.value)

    def visitConditional(self, cond):
        self.output(["if (", cond.conditional, ") {"],
                    cond.if_true,
                    "} else {",
                    cond.if_false,
                    "}")
    ...
    def visitBinaryOperation(self, bin_op):
        self.output(["(", bin_op.lhs, bin_op.op, bin_op.rhs, ")"])

    def visitUnaryOperation(self, un_op):
        self.output(["(", un_op.op, un_op.expr, ")"])

    def visitFunctionCall(self, func_call):
        self.output([func_call.fun_expr, "(", func_call.args, ")"])

    def visitFunctionDefinition(self, func_def):
        self.output(["def", func_def.name, "(", func_def.function.args, ") {"],
                    func_def.function.body,
                    "}")
    def visitPrint(self, pr):
        self.output(["print", pr.expr])

    def visitRead(self, re):
        self.output(["read", re.expr])
